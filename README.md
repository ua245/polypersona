...
# Sandbox runner (`polypersona/sandboxed`)

A second runner keeps the agent loop on your machine and gives each session a network-blocked Modal Sandbox,
so the Gemini key never enters it. It needs `GOOGLE_API_KEY` and `GEMINI_MODEL` in `.env`.
