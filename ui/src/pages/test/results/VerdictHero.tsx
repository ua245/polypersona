...
import { variantName } from '../../../lib/derive';
  ...
  return (
    <section aria-labelledby="pp-verdict" className="card" style={{
      ...
      <div className="mono" style={{ color: winner ? C.green : C.muted2, fontSize: 11, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
        Verdict · {agents} {agents === 1 ? 'agent' : 'agents'} · {variants.length} {variants.length === 1 ? 'variant' : 'variants'}
      </div>
      <h2 id="pp-verdict" style={{ margin: '8px 0 0', fontSize: 'clamp(32px, 7vw, 56px)', lineHeight: 1.05, fontWeight: 700, letterSpacing: '-0.03em', color: winner ? C.green : C.text }}>
        {winner ? `Variant ${winner.toUpperCase()} wins` : 'No clear winner'}
      </h2>
